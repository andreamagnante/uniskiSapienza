...
alter table `group_module`   add index fk_group_module_group (`group_oid`), add constraint fk_group_module_group foreign key (`group_oid`) references `group` (`oid_group`);
alter table `group_module`   add index fk_group_module_module (`module_oid`), add constraint fk_group_module_module foreign key (`module_oid`) references `module` (`oid_module`);


-- User_DefaultGroup [User2DefaultGroup_DefaultGroup2User]
alter table `user`  add column  `group_oid`  integer;
alter table `user`   add index fk_user_group (`group_oid`), add constraint fk_user_group foreign key (`group_oid`) references `group` (`oid_group`);
...
alter table `user_group`   add index fk_user_group_user (`user_oid`), add constraint fk_user_group_user foreign key (`user_oid`) references `user` (`oid_user`);
alter table `user_group`   add index fk_user_group_group (`group_oid`), add constraint fk_user_group_group foreign key (`group_oid`) references `group` (`oid_group`);


-- LezioneIstruttore [rel10]
alter table `lezione`  add column  `istruttore_oid`  integer;
alter table `lezione`   add index fk_lezione_istruttore (`istruttore_oid`), add constraint fk_lezione_istruttore foreign key (`istruttore_oid`) references `istruttore` (`user_oid_user`);


-- IndisponibileistrIstruttore [rel11]
alter table `indisponibileistr`  add column  `istruttore_oid`  integer;
alter table `indisponibileistr`   add index fk_indisponibileistr_istruttor (`istruttore_oid`), add constraint fk_indisponibileistr_istruttor foreign key (`istruttore_oid`) references `istruttore` (`user_oid_user`);


-- UserRegistrato_Prenotazioneskipass [rel12]
alter table `prenotazioneskipass`  add column  `userregistrato_oid`  integer;
alter table `prenotazioneskipass`   add index fk_prenotazioneskipass_userreg (`userregistrato_oid`), add constraint fk_prenotazioneskipass_userreg foreign key (`userregistrato_oid`) references `userregistrato` (`user_oid_user`);


-- UserRegistrato_Attivazioneskipass [rel13]
alter table `attivazioneskipass`  add column  `userregistrato_oid`  integer;
alter table `attivazioneskipass`   add index fk_attivazioneskipass_userregi (`userregistrato_oid`), add constraint fk_attivazioneskipass_userregi foreign key (`userregistrato_oid`) references `userregistrato` (`user_oid_user`);


-- UserRegistrato_Quotasocio [rel15]
alter table `quotasocio`  add column  `userregistrato_oid`  integer;
alter table `quotasocio`   add index fk_quotasocio_userregistrato (`userregistrato_oid`), add constraint fk_quotasocio_userregistrato foreign key (`userregistrato_oid`) references `userregistrato` (`user_oid_user`);


-- Amristorazione_Sconto [rel19]
alter table `sconto`  add column  `amristorazione_oid`  integer;
alter table `sconto`   add index fk_sconto_amristorazione (`amristorazione_oid`), add constraint fk_sconto_amristorazione foreign key (`amristorazione_oid`) references `amristorazione` (`amministratore_oid_user`);


-- Amristorazione_Menufisso [rel2]
alter table `menufisso`  add column  `amristorazione_oid`  integer;
alter table `menufisso`   add index fk_menufisso_amristorazione (`amristorazione_oid`), add constraint fk_menufisso_amristorazione foreign key (`amristorazione_oid`) references `amristorazione` (`amministratore_oid_user`);


-- Amristorazione_Menucarta [rel23]
alter table `menucarta`  add column  `amristorazione_oid`  integer;
alter table `menucarta`   add index fk_menucarta_amristorazione (`amristorazione_oid`), add constraint fk_menucarta_amristorazione foreign key (`amristorazione_oid`) references `amristorazione` (`amministratore_oid_user`);


-- UserRegistrato_Lezione [rel4]
alter table `lezione`  add column  `userregistrato_oid`  integer;
alter table `lezione`   add index fk_lezione_userregistrato (`userregistrato_oid`), add constraint fk_lezione_userregistrato foreign key (`userregistrato_oid`) references `userregistrato` (`user_oid_user`);
...
alter table `portafoglio`   add index fk_portafoglio_userregistrato (`userregistrato_oid`), add constraint fk_portafoglio_userregistrato foreign key (`userregistrato_oid`) references `userregistrato` (`user_oid_user`);
...
alter table `amimpianto_impianto`   add index fk_amimpianto_impianto_amimpia (`amimpianto_oid`), add constraint fk_amimpianto_impianto_amimpia foreign key (`amimpianto_oid`) references `amimpianto` (`amministratore_oid_user`);
