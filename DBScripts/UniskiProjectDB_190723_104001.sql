...
alter table `attivazioneskipass`  add column  `userregistrato_oid_user`  integer;
alter table `attivazioneskipass`   add index fk_attivazioneskipass_userregi (`userregistrato_oid_user`), add constraint fk_attivazioneskipass_userregi foreign key (`userregistrato_oid_user`) references `userregistrato` (`user_oid_user`);
